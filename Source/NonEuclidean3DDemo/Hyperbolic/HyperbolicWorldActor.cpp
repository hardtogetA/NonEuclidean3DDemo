...
AHyperbolicWorldActor::AHyperbolicWorldActor()
{
	...
	BaseMaterial = TSoftObjectPtr<UMaterial>(
		FSoftObjectPath(TEXT("/Game/Hyperbolic/M_Hyperbolic.M_Hyperbolic")));
}

void AHyperbolicWorldActor::BeginPlay()
{
	Super::BeginPlay();
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("HyperbolicWorldActor: no material at '%s'. Create it or set BaseMaterial."), *BaseMaterial.ToString());
	}

	BuildGeometry();
}

void AHyperbolicWorldActor::BuildGeometry()
{
	...
	for (int32 IX = -Half; IX <= Half; ++IX)
	{
		for (int32 IY = -Half; IY <= Half; ++IY)
		{
			for (int32 IZ = -Half; IZ <= Half; ++IZ)
			{
				...
				if (R > MaxKleinRadius || R < InnerRadius)
				{
					continue;
				}
	...
	UE_LOG(LogTemp, Log, TEXT("HyperbolicWorldActor: built %d vertices (%d cubes)."), Vertices.Num(), Vertices.Num() / 24);
}
